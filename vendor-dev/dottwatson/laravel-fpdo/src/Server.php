    ...
    public function getTableDefinition(string $database, string $table) : ?\Vimeo\MysqlEngine\Schema\TableDefinition
    {
        if(!isset($this->tableDefinitions[$database][$table])){
            ...
            if($tableDefinition){
                ...
                $parser = TableData::resolveParser($database,$table);
                ...
                $columnsConfig      = Arr::get($tableDefinition,'schema',[]);
                $queryTableCreate   = $pdoCls::buildTableData($table,$data,$columnsConfig,$this->pdo,$tableDefinition['charset']);

                $this->tablesCreationQuery[$table] = $queryTableCreate;

                $this->pdo->exec($queryTableCreate);
                $this->saveTable($database,$table,$data);
            }
        }
